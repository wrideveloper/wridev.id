<script lang="ts">
  ...
  import GameIcon from "~icons/ph/game-controller";
  ...
  const categories = [
    ...
    { name: "Game", value: "game", icon: GameIcon },
...
<div
  class="inline-flex items-center bg-[#F5F5F5] rounded-lg p-1 overflow-x-auto no-scrollbar max-w-full"
>
  {#each categories as category}
    <button
      on:click={() => ($selectedCategory = category.value)}
      class="flex items-center gap-1.5 rounded-md px-3 py-1.5 text-sm font-medium transition-all whitespace-nowrap
