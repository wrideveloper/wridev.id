...
<div
    class="group relative h-83 w-70.5 overflow-hidden rounded-xl bg-gray-200 cursor-pointer"
>
    <img
        src={image}
        alt={name}
    ...
    <div
        class="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100"
    ></div>
    ...
    <div class="absolute bottom-0 left-0 w-full p-4 flex flex-col justify-end">
        ...
        >
            {#each tags as tag}
                <span
                    class="rounded-md bg-white px-2 py-1 text-xs font-bold text-gray-900 shadow-sm"
                >
                    {tag}
                </span>
            {/each}
