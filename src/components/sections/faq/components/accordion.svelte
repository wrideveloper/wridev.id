...
<div class="flex flex-col w-full">
    <button
        ...
        on:click={onToggle}
    >
        <span class="text-sm md:text-base sm:text-lg lg:text-xl font-medium flex-1 pr-2">{question}</span>
        <div
            class="flex-shrink-0 transition-transform duration-300"
            style="transform: rotate({isOpen ? 180 : 0}deg)"
        >
            <CaretDown class="text-base md:text-lg sm:text-xl font-medium" />
        </div>
    </button>

    {#if isOpen}
        <div
            class="px-4 sm:px-6 py-3 sm:py-4 text-xs md:text-sm sm:text-base text-wri-gray-600"
